...
%yVac = repmat([1:num_labels], m, 1) == repmat(y, 1, num_labels);
all_combos = eye(num_labels);    
yVac = all_combos(y,:); 

J = (-1/m) * (sum(sum(yVac.*log(a3) + (1-yVac) .* log(1-a3))));

regTheta1 = Theta1(:,2:end);
regTheta2 = Theta2(:,2:end);

regularized = (lambda/(2*m)) * (sum(sum(regTheta1.^2))) + (sum(sum(regTheta2.^2)));
